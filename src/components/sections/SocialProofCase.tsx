...
export function SocialProofCase() {
  ...
  return (
    <section id="case" className="bg-canvas-2">
          ...
          <div className="mt-8 grid grid-cols-2 gap-4">
            {stats.map((stat) => (
              <Card key={stat.label} className="p-6">
                <p className="font-mono text-3xl font-medium text-brand-purple">{stat.value}</p>
        ...
        <Reveal delay={0.15} className="relative">
          <Card className="relative p-8">
            <div ref={ref}>
              ...
              <p className="mt-2 text-6xl font-black tracking-tight text-ink-strong">{value}</p>
                  ...
                  <div key={bar.month} className="flex h-full flex-1 flex-col justify-end gap-2">
                    <div
                      className={`w-full rounded-t-lg ${
                        bar.gradient ? "bg-gradient-primary" : "bg-ink/10"
                      }`}
                      style={{ height: bar.height }}
                    />
                    <p className="text-center font-mono text-xs uppercase tracking-widest text-ink/50">
                      {bar.month}
                    </p>
                  </div>
                ))}
          ...
          <div className="absolute -top-5 right-6 animate-float rounded-full bg-white px-4 py-2.5 shadow-card">
